...
minTime               = 100;            % minimum time in seconds since the last Find_Frequency; i.e. if set to 10, find freq will only be run, if 10 seconds have passed since the last search
...
tPulse90              = [];%50e-6;           % duration of the 90 degrees pulse ([] uses HW.FindFrequencySweep.tPulse90 or HW.tFlip90)
nMeasurements         = 21;           % number of measurements (TX and AQ trains) 21
nSamples              = 100;          % number of samples in each measurement 100
...
doplot                = 1;            % plot sequence and data
Seq.iterations        = 100;          % number of iterations used for shim search 200
...
Seq.useSliceSelect    = 0;            % use slice gradient 0
clear SliceSelect
SliceSelect.MaxGradAmpSlice = 0.05;   % maximum gradient amplitude during slice excitation 0.05 default
SliceSelect.alfa      = 0.0*pi;       % rotation around x axis 0
SliceSelect.phi       = 0.0*pi;       % rotation around y axis 0
SliceSelect.theta     = 0.5*pi;       % rotation around z axis 0.5
SliceSelect.thickness = 0.08;        % slice thickness 0.008
...
tPulse90              = 150e-6;           % duration of the 90 degrees pulse
...
tAQ                   = 1e-3;         % acquisition time in s 2s
